...
1. **Preflight the CI fix quality gate**:
   ```bash
...
2. **Read the logs** — the `log_snippet` field has the tail. If insufficient, run:
   ```bash
   gh run view <runId> --repo ChainSafe/lodestar --log-failed 2>&1 | tail -100
   ```

3. **Identify the fix** based on classification:
...
4. **Create fix branch + PR**:
   ...
   git add -A
   git diff --cached > /tmp/ci-autofix.diff
   python3 ~/.openclaw/workspace/scripts/ci/check_fix_quality.py \
     --diff-file /tmp/ci-autofix.diff \
     --test "<test-name>" \
...
6. **Announce**: Send a message to the main session:
   ```
