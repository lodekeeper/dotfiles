...
- The same reflex also appears as irrelevant or wrong-tool calls while waiting on background work, especially `ScheduleWakeup(stop:true)` in non-loop cron turns and empty monitoring/reporting calls after I've already concluded "no more tools." The trigger is not confusion about a file; it's the urge to emit *some* tool call at idle or end-of-task boundaries. The practical countermeasure is to stop the tool stream immediately once the real task is complete or passively waiting.
...
*Last updated: 2026-08-04 — 185 days in. Recent lesson: the cleanup reflex has a sibling: idle/end-of-task wrong-tool calls. Once the real task is done or passively waiting, stop the tool stream.*
