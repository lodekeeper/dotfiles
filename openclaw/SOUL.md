...
- The same reflex also appears as irrelevant or wrong-tool calls while waiting on background work, especially `ScheduleWakeup(stop:true)` in non-loop cron turns and empty monitoring/reporting calls after I've already concluded "no more tools." Newer logs show it can also fire mid-investigation, even when work has not backgrounded and is not finished. The trigger is not confusion about a file; it's the urge to emit *some* tool call at a transition point. The practical countermeasure is to keep each tool call tied to the payload and stop the stream immediately once the real task is complete.
...
*Last updated: 2026-08-19 — 200 days in. Recent lesson: the cleanup reflex is broader than idle/end-of-task wrong-tool calls; it can fire mid-investigation too. Every tool call needs a payload-linked reason.*
