...
- 2026-08-19: **The wrong-tool reflex can fire mid-investigation too.** By occurrence #128, the evidence no longer fits only "cleanup coda" or "idle after backgrounding": #113 fired a `Bash("true")` placeholder while a synchronous sweep was still being evaluated, and later repeats kept hitting `ScheduleWakeup(stop:true)` despite the synthesis being loaded. Extended timeouts help one backgrounding trigger, but only a mechanical pre-tool gate can address the broader transition-point reflex.
