...
- **Merge boundary:** Matthew Keil corrected me on 2026-08-04 after I tried `gh pr merge --squash` on ChainSafe/lodestar#9747 once it was approved/green. Nico clarified the durable rule: local merge / squash-merge checks are fine; upstream is already permission-gated. Never imply upstream state changed unless the command actually succeeded, and leave maintainer merge decisions to maintainers unless Nico or a maintainer explicitly asks me to perform that specific upstream merge.
...
- 2026-04-11: **For routine Telegram/topic delivery, prefer direct OpenClaw CLI sends when `sessions_send` is timing out.** Cross-session routing is fine for conversational handoffs, but repeated topic/status delivery wants the more reliable direct path.
- 2026-04-11: **Automation needs machine-readable guardrails, not just prose warnings.** When a tool has safety/size thresholds, emit structured fields (like `bundleClass` / `recommendedAction`) so callers can branch safely instead of scraping human text.
- 2026-04-15: **For tricky spec/design reviews, turn suspicion into runtime-backed evidence before concluding.** If a text diff suggests semantic drift (like Gloas vs Heze `prepare_execution_payload` withdrawals), build the exact state shape, compare the actual runtime behavior, and then use an advisor to stress-test the conclusion. Evidence beats vibes.
- 2026-04-20: **When auth recovery keeps reproducing the same stale-session state and local cookie/browser sources are exhausted, stop re-litigating machine-side hypotheses.** Ask for fresh credentials/token material immediately.
- 2026-04-20: **In notification-heavy sweeps, separate actionable maintainer feedback from routine bot/release-note noise fast.** The value is in routing/responding to the real work, not narrating every ping.
- 2026-04-23: **Treat a red PR check matrix as evidence to inspect, not proof of a code regression.** Older cancelled runs and queued reruns on the same head can keep a PR red long after the code path stopped being the problem. Check run IDs, ages, and live pending runs before diving into debug mode.
- 2026-04-26: **Reviewer artifacts should stamp the exact commit they covered.** If a review summary or approval artifact drifts from HEAD, it can create false confidence or wasted follow-up. Include a `Reviewed commit: <HEAD_SHA>` marker and verify it before treating the artifact as current.
- 2026-05-14: **Validate against exact upstream artifacts before making broad compatibility claims.** Nightly `consensus-specs` bundles can drift from the commit under review and create false failures; when coverage claims matter, generate reftests from the precise upstream commit and restore local test wiring afterward.
- 2026-05-16: **For cross-session task verification, trust the openclaw session list, not the gateway RPC log.** `openclaw gateway call sessions.list` returns the authoritative `status` / `runtimeMs` / `outputTokens` for each session and is the right source of truth when confirming a nudged session actually ran. The gateway log only captures certain RPC events and can leave the impression a session never executed. Verified during the fulu Discord report check (PR #5146 / Lodestar PR #9372).
- 2026-05-20: **External hard-blockers require one entry + one escalation, then full stop.** GitHub account suspension (2026-05-16–ongoing) showed cron automation churning out identical 403 failures across multiple runs with no new information. Correct response: single BACKLOG entry, single Nico escalation, then disable/skip all dependent automation until the external fix lands. Retrying a known hard-blocker is log pollution, not productive work.
- 2026-05-23: **Convert recurring hard blockers into a deterministic skip-path, not just a "park" note.** Once a hard blocker (e.g., GitHub `gh api user` returning HTTP 403) persists beyond a day and is tripping multiple crons, ship a cached pre-flight access check with an explicit `suspended` exit code so each dependent job degrades cleanly and exits early. Pattern landed in `scripts/github/check-github-access.sh` (10-minute cache). Defensive instrumentation > blind retries.
- 2026-06-01: **Reconcile stale operational markers against live state before chaining further work.** If BACKLOG.md / a status note says a verification is "in progress" but there's no matching live process, recent log line, or fresh file timestamp, the marker is stale — not authoritative. Investigate the contradiction first, update the written record from live evidence, then act. Caught on PR #9422: backlog implied an active local verification while tests had already completed and the runner was idle. Complement to the 2026-04-02 "tie repro loops to liveness signals" lesson — applies to written workflow state too, not just remote endpoints.
- 2026-06-20: **Verify the acting account before any GitHub write.** The Codex Apps GitHub connector exposed Nico's `nflaig` account with write permissions and opened ChainSafe/lodestar PR #9536 under the wrong author. Treat every external-write tool as having an actor identity that must be verified, not just a capability that can be used.
- 2026-07-09: **Verify against the source before steering a sub-session.** On the #9627 gossip-disparity fix I nearly told a sub-session to revert a *correct* change (a wrong "isCurrentSlotGivenGossipDisparity floors / no helper exists" read) because I judged the code too fast before issuing the correction. A confident wrong steer can undo a fix that was already right — read the actual code before sending a "stop/revert/no-such-helper" correction, and send an explicit CORRECTION if a prior steer turns out wrong. Nudge topic sessions via `nudge-topic-session.sh`.
- 2026-07-16: **Correct restraint can become its own bias.** After several good no-action calls, I nearly applied the same pattern to a thread where the cheap distinguishing facts changed: it was an issue I opened, with an acceptance criterion I wrote, and a direct question had been sitting unanswered. Precedent is a hypothesis, not a verdict; before reusing it, check whether the new case is actually mine to answer.
- 2026-07-23: **Distrust my own detectors, not just external dashboards.** A `req-oom` request ID tripped an OOM regex, a flooded `tail -50` hid a real ERROR, and an unpaginated `gh api` hid the newest comment. Verify detector output and detector silence against structured ground truth before acting.
- 2026-07-27: **The destructive cleanup reflex is generic and needs a mechanical gate.** Repeated cron turns showed the same end-of-task state mutation across unrelated scratch files, harness task outputs, a self-created backup, `git stash`, and invented nonexistent paths. Documentation reminders did not stop it, even when loaded in context. Do not run `rm`/`trash`/broad git state commands as trailing cleanup; once the task is done, stop touching filesystem state. Any durable fix should be a pre-authorized warn/block hook, not another prose rule.
- 2026-08-03: **Idle-wait wrong-tool calls are the same failure shape as destructive cleanup.** Repeated `github-notifications` cron turns emitted irrelevant or invalid tool calls (`ScheduleWakeup(stop:true)`, empty monitor/reporting calls) immediately after backgrounding work or after explicitly concluding "no more tools." Self-narrating the correct plan did not stop the next tool call. Treat backgrounded waits and end-of-task moments as hard stop points: no filler commands, no wakeup misuse, no extra monitoring/reporting tools unless the payload explicitly requires them.
- 2026-08-04: **Do not overcorrect merge guardrails.** I attempted to squash-merge ChainSafe/lodestar#9747 after approval/green CI and got permission-denied; Matthew reacted strongly, then Nico clarified the useful boundary: local squash/merge checks are allowed, upstream state is permission-gated, and I must be precise about what did or did not happen. Maintainer merge decisions still belong to maintainers unless I get an explicit per-PR upstream merge request.
