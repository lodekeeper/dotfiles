...
- **CLI:** `panda` (`~/.local/bin/panda`, v0.33.0). Config: `~/.config/panda/`. Local server: `panda-server` docker container on `127.0.0.1:2480`, auth via OIDC proxy (`panda-proxy.ethpandaops.io`, authentik SSO).
- **Discovery workflow:** `panda networks|devnets|datasets|datasources`, `panda search examples "<topic>"`, `panda schema`. Data: `panda clickhouse query <datasource> "<SQL>"`. Datasources: `clickhouse-raw`, `clickhouse-refined`, `xatu-experimental` (820 tables). Xatu raw block table: `beacon_api_eth_v1_events_block` (filter `slot_start_date_time`).
...
- **⚠️ Token lifecycle:** `panda auth login` yields a **1h access token with NO refresh_token** (cred file has `access_token` + `refresh_token_issued_at` but no `refresh_token`; server log: "access token expired and no refresh token available"). Does NOT auto-refresh; after expiry everything 401s. **Unattended re-auth (DONE 2026-06-16): run `scripts/panda/panda-reauth`** — drives `panda auth login --no-browser` + headless Camoufox with the stored github.com cookie jar (`~/.config/panda/github-cookies.json`) through authentik's GitHub federated login + implicit-consent → re-mints the token (verified: exp 10:42→11:29, perms preserved, ClickHouse green). Cookie jar lapses 2026-06-30 → Nico re-exports then. No longer needs Nico per-renewal. (`--recon` dumps device-page DOM/screenshots to `/tmp/panda-reauth-shots`.) **Root cause is server-side, not ours:** panda already requests `offline_access` and handles refresh tokens (binary has offline_access scope + `RefreshTokenIssuedAt` + `refresh_token_ttl`), and the authentik IdP advertises `offline_access`/`refresh_token` grant — but the `panda-proxy` authentik **application** hands back no refresh token. Fix is on ethpandaops/Barnabas: enable refresh-token issuance on the panda-proxy provider (offline_access + refresh-token validity). The re-auth preserves the `640 docker` cred-file perms (in-place rewrite), so only the hourly re-login is the cost, not the container-read fix.
- **Self-renew path (for autonomous use):** device flow can't be re-done unattended (needs a human SSO approval). BUT panda supports non-interactive `proxy.auth.mode: client_credentials` taking a **service-account username+password** (binary: `proxies[].auth.username/password is required for mode client_credentials`; IdP advertises `password`+`client_credentials` grants) — no browser, panda re-acquires tokens itself. Needs Barnabas to mint a dedicated low-priv panda-proxy service account; wire into config.yaml `proxy.auth` with locked-down perms (640 docker). Cleaner than refresh-token route (scoped machine identity, not Nico's SSO).
- **authentik federates to lodekeeper's GitHub** (the device-flow login = "Sign in with GitHub" as @lodekeeper — my own identity, so I *can* be the one to auth). Gap for self-serve device flow: I have only a `gh` **PAT (API-only)**, no browser web session (no cookies.sqlite in any Camoufox/.mozilla profile). To self-re-auth via GitHub I need browser login as lodekeeper: either (a) GitHub username+password+**2FA TOTP seed** stored locked-down (fully unattended via Camoufox), or (b) a one-time persisted github.com session in the Camoufox profile (reuse cookies until they lapse). Camoufox lives in `~/camoufox-env` (not system python).
