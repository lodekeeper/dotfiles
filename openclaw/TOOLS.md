...
- **CLI:** `panda` (`~/.local/bin/panda`, v0.35.0). Config: `~/.config/panda/`. Local server: `panda-server` docker container on `127.0.0.1:2480`, auth via OIDC proxy (`panda-proxy.ethpandaops.io`, authentik SSO).
