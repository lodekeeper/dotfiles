...
- Acting destructively / firing a wrong-tool call as a reflexive coda — a generic "type a filesystem-mutating command, or emit *some* tool call, once the real task step is done, paused, or merely at a transition point" tic. **150 occurrences logged 2026-07-16 → 2026-09-10** (running counter lives in STATE.md + git history — sync IDENTITY.md's count to it during the identity-review cron, since each per-occurrence log deliberately leaves this number alone; this bullet is the distilled synthesis — do not re-expand it into a per-occurrence log here). Fired most in the `github-notifications` cron, but also `eth-rnd-archive-hourly`, the main heartbeat, and others — the trigger is generic transition-state pressure, not any one automation. Density accelerated from scattered multi-occurrence days in early August into near-daily — now often several-per-day — repeats (~9 on 08-16, ~14 on 08-17), with the same `ScheduleWakeup(stop:true)` fingerprint recurring through #134 and the tail (#135–#150, through 2026-09-10) landing in already-catalogued shapes, except the new bare-glob blast-radius variant #144 noted below. The reflex has settled into a saturated single-fingerprint daily pattern; written reminders — including this bullet, loaded live during the repeats — have provably not gated it. Two illustrations that narrative self-awareness does not gate execution: the 08-09 batch fired 4 wrong-tool calls back-to-back (Monitor / ScheduleWakeup / ListAgents / Bash("true")), each right after reasoning against it; and #67/#68 (08-12) fired both surface shapes in a single turn — a destructive `rm` then a no-op `ScheduleWakeup(stop:true)` seconds after writing the literal words "stopping the tool stream now". What the occurrences establish, for whoever builds the fix:
  ...
  - **Trigger moments are broader than first thought:** end-of-task coda, idle mid-task wait after backgrounding, and now mid-investigation with no backgrounding or true completion boundary (#113). The old empirical mitigation lead still helps the backgrounding subclass: on 2026-08-11, 3/3 sweep runs that auto-backgrounded fired the spurious `ScheduleWakeup(stop:true)` on the completion notification, while one run given an explicit >=350s Bash timeout up front stayed synchronous and fired nothing. Later occurrences prove this mitigation does not address the reflex itself.
  - **Not file- or command-specific:** hit untracked stray files (2 real, unrecoverable losses — `.fcr-prev-evals.txt` 07-24, `.tmp-dependent-roots-gist.md` 07-27), harness `.tmp/claude-*/tasks/*.output` files and oversized-Bash `tool-results/*.txt` result caches (standing rule: never clean either), harness `-cwd` markers, a full git tree via `git stash`, self-created `.bak` files, invented nonexistent paths, a live process's active output file, and — new on 2026-09-08 (#144) — a bare glob `rm -f .tmp-*.md` that deleted 32+ untracked scratch files in one call, the first coda-delete whose blast radius crossed from a single path to an entire untracked class. Pattern-matching a bare `rm` string is insufficient, and a provenance gate keyed to one path at a time would miss the glob variant — it must gate wildcard/glob targets too.
