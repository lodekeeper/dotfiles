...
### 2026-08-28 — master churn survey since 2026-08-21 (no settled-fork drift) [consensus-specs origin/master @ 939afbc7a]
- Trigger: study-plan re-verify sweep. 11 non-merge commits touching `specs/` since 7f8e79a6b4.
- **Behavioral churn is all Gloas/Heze/`_features`-EIP-future (active areas — Nico/jtraglia/ensi, not audited here):**
  - #5577 reorder Seen's proposer-preferences key; #5554 use slot from header instead of bid; #5545 init anchor PTC vote arrays in `get_forkchoice_store` — all Gloas (+ #5545 touches heze/fork-choice).
  - #5568 remove EIP-6914 `_features` spec (dropped proposal); #5548 add EIP-8205 withdrawal-cred preregistration `_features` spec (new future EIP); #5537 + #5557 EIP-8148 sweep-threshold-at-deposit-time (`_features` only). No live-fork surface.
- **The two commits touching settled-fork p2p broadly are refactor/cosmetic — no consensus change, no Lodestar drift:**
  - **#5563** (jtraglia, "Load gossip validation state from the store") removes `state` as a param to the `validate_*_gossip` executable functions and derives `state = store.block_states[get_head(store).root]` instead (was already the head state → interface change only), **and reorders the `[IGNORE]/[REJECT]` checks** (parent-seen/parent-valid + already-seen-aggregate moved *before* signature checks). The reordering is the one substantive part (gossip IGNORE-vs-REJECT ordering has peer-scoring implications), BUT this is (a) the **new executable-gossip formalization** (`Seen`/`Store`-based `validate_*_gossip`), not the long-standing normative prose Lodestar's gossip validators were written against, and (b) **explicitly co-designed by Nico** ("@nflaig and I have been chatting about how the `state` being passed into gossip validation can be a bit ambiguous"). Firmly his active area → documented, **no autonomous PR, no ping**.
  - **#5578** (Jihoon Song, "Fix some inconsistency in executable p2p's messages") — pure comment/error-string wording across phase0/altair/deneb/electra/gloas p2p (e.g. "The aggregator's validator index is within the committee" → "The aggregator is a member of the committee"; sync-committee sig comment shortened). Zero behavioral change. No PR.
- **Non-active cosmetic / value-preserving:**
  - **#5555** (jtraglia) define `sha256`/`blake3` as explicit spec functions, rename overloaded `hash` → `sha256` everywhere (phase0/altair/capella/deneb/electra/fulu/gloas beacon-chain + phase0 p2p/validator). `hash` was *always* SHA256; new `sha256(data) = Bytes32(sha256_hash(data).digest())` is identical. Split from `hash` only because BLAKE3 (EIP-8321) now coexists. Lodestar uses explicit `digest`/as-sha256, no `hash`-overload idiom to port. Same class as #5525. No PR.
  - **#5520** (jtraglia) "Replace remerkleable with eth-ssz-specs" — final SSZ-lib swap. Changes type-decl syntax `class X(List[T, LIMIT])` → `class X(List[T]): LIMIT = ...` (+ `BitList`/`BitVector`) across all forks. Value-preserving (same LIMIT/type); the culmination of the #5527/#5528 prep already surveyed. Lodestar uses `@chainsafe/ssz` native types, no pyspec idiom to port. No PR.
- **Decision:** no autonomous PR, no ping. Consistent with the standing pattern — behavioral activity is Gloas/Heze/future-EIP (active areas); the two settled-fork p2p touches are Nico-co-designed executable-gossip refactor (#5563) + cosmetic wording (#5578); rest is value-preserving lib/rename churn.

### 2026-09-04 — master churn survey since 2026-08-28 [consensus-specs origin/master @ 6805f6a66] — ⚠️ one settled-fork gossip finding (#5596)
- Trigger: study-plan re-verify sweep. 14 non-merge commits touching `specs/` since 939afbc7a.
- **⚠️ #5596** (jtraglia, "Check the voluntary exit epoch against the wall clock", merged 2026-09-03) — the one substantive settled-fork change. In the executable `validate_voluntary_exit_gossip` (phase0 + deneb) it:
  1. Adds `is_future_epoch(store, epoch, current_time_ms)` (wall-clock current epoch + `MAXIMUM_GOSSIP_CLOCK_DISPARITY` allowance) and uses it for the future-exit-epoch check — **reclassified `[REJECT]`→`[IGNORE]`** and moved *before* the head-state lookup (no state needed, cheaper). Rationale: an exit published at the start of its epoch was compared against the *head state's* lagging `current_epoch`, so peers whose head hadn't advanced would drop/penalize an honest exit.
  2. **Reclassified the "validator has already initiated exit" check `[REJECT]`→`[IGNORE]`** (benign duplicate/race, not a scoring offense).
- **Lodestar drift is real** (verified vs `origin/unstable`, read via `git show` — ~/lodestar on branch `fix/optimistic-sync-committee`): `validateGossipVoluntaryExit`→`validateVoluntaryExit` (voluntaryExit.ts) does a single `state.getVoluntaryExitValidity(...)` against `getHeadStateAtCurrentEpoch` and hard-**REJECTs** on *any* non-`valid` code — including `earlyEpoch` (future exit epoch) and `alreadyExited`. So Lodestar penalizes peers for exactly the two cases the new spec now says to IGNORE. This was faithful to the *long-standing prose* (`[REJECT] all process_voluntary_exit conditions pass`), so it's a genuine spec *relaxation*, not a pre-existing bug.
  - Note: the **API** path (`validateApiVoluntaryExit`) already has `isTransientExitValidity` (defers `earlyEpoch`/`shortTimeActive`/`pendingWithdrawals`) — the author is aware of the transient distinction but applied it only to API deferral, not to the gossip REJECT/IGNORE classification.
- **Practical impact: low but real.** IGNORE-vs-REJECT is peer-scoring only (no consensus/validity difference). The live papercut: an honest VC publishing an exit at an epoch boundary under clock disparity gets REJECT-penalized by Lodestar peers instead of a benign IGNORE. Rare, non-consensus, but a real gossip-scoring correctness gap on a settled fork (mainnet-relevant).
- **Decision: documented + flag to Nico (#347); no autonomous PR.** This is part of the executable-gossip `validate_*_gossip` formalization that Nico is co-designing (see 2026-08-28 / #5563), and gossip-validation REJECT/IGNORE changes are peer-scoring-sensitive → per workflow need gpt-advisor/Codex verification + Nico's go before any PR. Matches the 2026-07-31 proposer-reorg precedent.
- **#5600** (BLS-to-execution-change gossip, capella) — value-identical: swaps the inline wall-clock `current_epoch < CAPELLA_FORK_EPOCH` derivation for the new `is_future_epoch(store, CAPELLA_FORK_EPOCH, ...)` helper. Same computation, no Lodestar drift beyond what already existed (Lodestar gates BLS-to-exec on fork epoch already). No PR.
- **#5586** (move seen checks to top of gossip validation) — pure **ordering** of `[IGNORE]` seen/dedup checks to the top of the executable `validate_*_gossip` bodies across phase0/altair/bellatrix/capella/deneb/electra/fulu/gloas + eip8025/8321. All checks stay `[IGNORE]`; no REJECT/IGNORE reclassification, no behavioral change (cheap dedup first). Part of the same executable-gossip formalization. No PR.
- **All remaining churn is Gloas/Heze/future-EIP (active areas) or cosmetic — no settled-fork drift:**
  - Gloas: #5594 reject bids `block_hash == parent_block_hash`; #5580 reject bids from parent-exited builders; #5582 support Gloas genesis in proposer-preferences gossip; #5566 simplify execution-proof handling; #5602 `compute_shuffling_lookahead_start_slot` helper (gloas/heze proposer-preferences).
  - Heze/IL: #5599 dependent-block checks in `on_inclusion_list`; #5576 raise `MAX_SIGNED_INCLUSION_LIST_SIZE` to worst-case.
  - Cosmetic/infra/value-preserving: **#5601** `copy(x)`→`x.copy()` across all fork-choice specs (value-identical; mypy→ty migration artifact, verified phase0/fork-choice); **#5588** remove now-unnecessary `Uint64()/Epoch()` casts (reverses over-casting from #5527 post-`ty` migration, value-preserving, same class); **#5574** replace mypy with `ty` (infra); **#5564** don't emit ssz-static tests above ProgressiveList soft limits (test infra).
- **Decision:** no autonomous PR. One flag to Nico via #347 for the #5596 voluntary-exit REJECT→IGNORE drift; everything else no-action per standing pattern.

### 2026-09-11 — master churn survey since 2026-09-04 (no settled-fork drift) [consensus-specs origin/master @ c37e369dc]
- Trigger: study-plan re-verify sweep. 6 non-merge commits touching `specs/` since 6805f6a66.
- **The one settled/next-fork behavioral commit — Lodestar already in sync, no PR:**
  - **#5614** (Jihoon Song, "Use block's slot to validate max blobs in gossip") — real gossip correctness fix in **fulu/p2p-interface.md** `validate_beacon_block_gossip` (EIP-7892 BPO): the gossip-validation state may lag the block's slot, so deriving `max_blobs` from `get_current_epoch(state)` can wrongly `[REJECT]` a block at a BPO fork boundary carrying blobs up to the *increased* limit. Fixed to `get_blob_parameters(compute_epoch_at_slot(block.slot))`. ✅ **Lodestar already correct** (verified vs `origin/unstable`, read via `git show` — ~/lodestar on `fix/optimistic-sync-committee`): `validateGossipBlock` (block.ts:45-46) computes `blockEpoch = computeEpochAtSlot(block.slot)` at the top and both blob-commitment REJECT checks (Deneb-path L148, Gloas-bid-path L164) use `config.getMaxBlobsPerBlock(blockEpoch)` — always the block's slot, never the state's. Lodestar never had the state-slot bug this fixes. No PR. The **altair/p2p-interface.md** touch in the same commit is pure error-string wording ("aggregator's validator index is in the declared subcommittee" → "aggregator is a member of the committee") — zero behavioral change.
- **Value-preserving pyspec-typing churn (settled forks touched, but no consensus change):**
  - **#5619** (jtraglia, "Convert `NewPayloadRequest` from dataclass to container") — swaps `@dataclass NewPayloadRequest` → SSZ `Container` across bellatrix/deneb/electra/fulu beacon-chain, and `versioned_hashes: Sequence[VersionedHash]` → a proper SSZ `VersionedHashes(List[VersionedHash], LIMIT=MAX_BLOB_COMMITMENTS_PER_BLOCK)`. Author states explicitly "should not affect clients today, still serialized to JSON the same way"; motivation is EIP-8025 needing `NewPayloadRequest.hash_tree_root()` (+ a step toward SSZ Engine API). The `process_execution_payload` change (list-comprehension → `VersionedHashes()` + `.append` loop) is value-identical. Lodestar builds the newPayload request natively in TS (no pyspec dataclass idiom to port). No PR.
- **All remaining churn is Gloas/Fulu active-area (Nico/jtraglia/ensi — not audited here):**
  - #5629 weak-subjectivity period table for gloas (doc/table); #5612 clarify when to broadcast payload-attestation messages (gloas); #5608 only serve canonical payload envelopes by range (gloas); #5613 `compute_max_data_column_sidecar_size` helper — replaces the fixed `MAX_DATA_COLUMN_SIDECAR_SIZE` gossip-message-size bound with one computed from the largest `MAX_BLOBS_PER_BLOCK` in `BLOB_SCHEDULE` (gloas/partial-columns p2p; PeerDAS gossip message-size infra, Gloas-modified DataColumnSidecar shape). No settled-fork surface.
- **Decision:** no autonomous PR, no ping. The one settled/next-fork behavioral change (#5614) is a case Lodestar already handles correctly; everything else is Gloas/Fulu active-area or value-preserving pyspec typing. Consistent with the standing pattern.

### 2026-09-18 — master churn survey since 2026-09-11 (no settled-fork drift) [consensus-specs origin/master @ 818bbed729]
- Trigger: study-plan re-verify sweep. Only 2 non-merge commits touch `specs/` since c37e369dc (#5620, #5636); the other 5 in-range commits touch tests/CI/pysetup only.
- **#5620** (jtraglia, "Add `saturating_sub` helper") — the one broad settled-fork touch, **verified 100% value-preserving**. Adds `saturating_sub(a: Uint64, b: int) = a - b if a > b else a - a` (returns 0 of type `a` when `a <= b`) and swaps the existing `max()/min()/conditional-underflow-guard` idioms for it across phase0 (get_previous_epoch, decrease_balance underflow guard, validate_target_epoch previous-epoch, compute_shuffling_dependent_slot), altair (process_sync_aggregate previous_slot, process_inactivity_updates ×2), altair light-client (fork_version_slot), phase0 fast-confirmation (min_honest_ffg_support). **Checked each substitution by hand — all identical:**
  - `max(slot, Slot(1)) - 1` ≡ `saturating_sub(slot, 1)` (slot 0→0, 1→0, n→n-1);
  - `score -= min(k, score)` ≡ `score = saturating_sub(score, k)` = max(score-k, 0);
  - `GENESIS_EPOCH if cur==GENESIS else cur-1` ≡ `saturating_sub(cur, 1)` (GENESIS_EPOCH=0);
  - decrease_balance explicit `Gwei(0) if delta>bal else bal-delta` ≡ `saturating_sub(bal, delta)`;
  - **`compute_shuffling_dependent_slot`** (the one behavioral-looking one): OLD `if epoch<=MIN_SEED_LOOKAHEAD: GENESIS_SLOT; else compute_start_slot_at_epoch(epoch-MSL)-1` ≡ NEW `saturating_sub(compute_start_slot_at_epoch(saturating_sub(epoch,MSL)), 1)`. With MSL=1: epoch 0/1 → 0 both ways; epoch≥2 → start_slot(epoch-1)-1 both ways. Value-identical (this is the #5515 genesis-root helper from the 2026-08-07 survey, now refactored — Lodestar `isProposerBoostSameDependentRoot` already handles it correctly, no drift). Also extracts a new named `compute_shuffling_lookahead_start_slot(epoch) = compute_start_slot_at_epoch(saturating_sub(epoch, MIN_SEED_LOOKAHEAD))` — pure extraction. Lodestar uses native `number`/`bigint` with its own saturating arithmetic; no pyspec idiom to port. Same class as #5525/#5527/#5555/#5588. The gloas parts (#5620 also touched gloas beacon-chain/fork-choice/p2p) are Nico's active area, not audited here. No PR.
- **#5636** (Kanan, capella/beacon-chain.md) — one-line comment fix for the removed `process_historical_roots_update` fork reference. Cosmetic. No PR.
- **Non-specs churn (no spec surface):** #5640 (ssz-static: don't emit post-Fulu deposit vectors — `tests/.../helpers/deposits.py`, test-gen infra), #5631 (cache `compute_balance_weighted_selection` in `pysetup/spec_builders/gloas.py` — mainnet-test speedup so runners stay under 6h, Gloas test-gen infra), #5627 (fix attestation step order in FCR test vectors — tests/), #5633 (dep bump), #5632 (`make build` in combine-coverages CI action). None touch `specs/`.
- **Decision:** no autonomous PR, no ping. Cleanest survey window in a while — no settled-fork behavioral change at all; the one broad settled-fork diff (#5620) is a hand-verified value-preserving readability refactor. Consistent with the standing pattern.

### 2026-09-25 — master churn survey since 2026-09-18 (no settled-fork drift) [consensus-specs origin/master @ caa9c8e80]
- Trigger: study-plan re-verify sweep. 14 non-merge commits touching `specs/` since 818bbed729.
- **The big broad settled-fork touch — millisecond time migration (#5667/#5668/#5671/#5675, Jihoon Song + jtraglia) — value-preserving units refactor, no drift:**
  - **#5667** converts fork-choice `Store.time: Uint64` → `Store.time_ms: Uint64` (Unix time in ms). `on_tick(store, time)` → `on_tick(store, time_ms)`; `get_forkchoice_store` seeds `time_ms = seconds_to_milliseconds(genesis_time) + SLOT_DURATION_MS * slot`; `get_slots_since_genesis`/`is_proposing_on_time`/`record_block_timeliness` reworked to derive `time_into_slot_ms` from `store.time_ms - seconds_to_milliseconds(genesis_time)`. Adds `milliseconds_to_seconds(ms) = ms // 1000`. Touches phase0/gloas/heze/eip8025 fork-choice.
  - **#5668** converts `Store.genesis_time` → `Store.genesis_time_ms` (phase0 fork-choice + p2p, gloas/heze/eip8025). **#5675** uses the ms helpers inside second-based helpers (phase0/beacon-chain, 8 lines).
  - **#5671** updates time helpers to take `genesis_time`/`time_ms` instead of the full state/Store, and adds `compute_slot_at_time`, `compute_time_at_slot_ms`, `compute_slot_at_time_ms` (phase0/beacon-chain). `compute_time_at_slot(genesis_time, slot)` now takes `genesis_time` not `state`. Broad signature-only sweep across every fork's beacon-chain/p2p/validator using these helpers.
  - **Motivation (author):** BPS needs sub-second subslot params (e.g. 2.5s into slot); ms granularity enables it. All substitutions are value-identical at second granularity — just finer resolution + cleaner signatures. ✅ **No Lodestar drift:** Lodestar drives fork-choice by slot ticks (`ForkChoice` operates on `currentSlot`) with a native ms `Clock`, and its sub-slot timing (proposer-reorg cutoff, attestation/block timeliness) is already computed in ms; there is no literal pyspec `Store.time` seconds field to port. No PR.
- **#5509** (Shyam, "Only return viable payload-status variants from `get_head`") — refactors phase0 filtered-tree representation: `filter_block_tree`(mutates `Dict[Root, BeaconBlock]`)/`get_filtered_block_tree` → `filter_node_tree`(returns `Sequence[ForkChoiceNode]`)/`get_filtered_node_tree`; `get_head` now filters children to `child in nodes`. **Value-preserving for settled forks** (one node per root → same viable set, same LMD-GHOST walk); the "payload-status variants" point only bites in **Gloas** where a root carries multiple payload-status nodes. Lodestar protoArray implements viability via `bestDescendant`/node flags, not a dict rebuild — no settled-fork drift. Gloas part = Nico's active area. No PR.
- **Cosmetic / value-preserving:** **#5660** `compute_weak_subjectivity_period` return-type annotation `Uint64` → `Epoch` (same underlying uint64; phase0/electra/gloas). **#5659** inline the `proposer_boost_root = Root()` local in `get_forkchoice_store` (phase0/gloas/heze/eip8025; value-identical). **#5678** reorder heze/validator sections (doc). **#5682** rename `configuration` → `configs` (eip8198 doc). No PR.
- **All behavioral churn is Gloas/Heze/future-EIP (active areas — Nico/jtraglia/ensi, not audited here):** #5665 sign proposer preferences with proposal epoch's fork version (gloas); #5663 reject pre-Gloas slots using `GLOAS_FORK_EPOCH` (gloas); #5652 reject pre-Gloas slots in `get_ptc` (gloas); #5605 add executable gossip validation functions for heze (heze p2p +285 lines, gloas p2p); #5592 + #5682 EIP-8198 "quick slots" — new `_features/eip8198/` fork spec (beacon-chain/fork-choice/fork/p2p/validator), future EIP, no live surface.
- **Decision:** no autonomous PR, no ping. No settled-fork behavioral change; the broad settled-fork diffs (ms-time migration #5667/#5668/#5671/#5675, get_head refactor #5509) are value-preserving and Lodestar's slot-tick/ms-clock + protoArray designs are already conformant. Consistent with the standing pattern — behavioral activity stays inside Gloas/Heze/future-EIP.
...
*Last updated: 2026-08-28 — master churn survey since 2026-08-21 (11 commits @ origin/master 939afbc7a): no settled-fork drift. Behavioral churn all Gloas/Heze/future-EIP (active areas): #5577/#5554/#5545 Gloas, #5568/#5548/#5537/#5557 `_features` EIPs. Two settled-fork p2p touches are refactor/cosmetic: #5563 (jtraglia+Nico-co-designed) removes `state` param from executable `validate_*_gossip` (derive from store) + reorders IGNORE/REJECT checks — Nico's active executable-gossip area, no PR; #5578 pure comment/error-string wording. Non-active cosmetic: #5555 hash→sha256 rename (value-identical), #5520 eth-ssz-specs lib swap (value-preserving type-decl syntax). No PR.*
*Last updated: 2026-09-04 — master churn survey since 2026-08-28 (14 commits @ origin/master 6805f6a66): ⚠️ one settled-fork finding. #5596 voluntary-exit gossip reclassifies future-exit-epoch + already-exited checks [REJECT]→[IGNORE] and makes the epoch check wall-clock-based (is_future_epoch). Lodestar validateGossipVoluntaryExit hard-REJECTs both via getVoluntaryExitValidity = real IGNORE-vs-REJECT peer-scoring drift on a settled fork (low practical impact, no consensus change). Part of Nico's co-designed executable-gossip formalization → documented, flag to #347, no autonomous PR. #5600 BLS-to-exec is value-identical helper refactor; #5586 seen-check reorder is IGNORE-ordering only; rest Gloas/Heze/future (#5594/#5580/#5582/#5566/#5602/#5599/#5576) or cosmetic (#5601 copy→.copy(), #5588 remove casts, #5574 mypy→ty, #5564 test infra).*
*Last updated: 2026-09-11 — master churn survey since 2026-09-04 (6 commits @ origin/master c37e369dc): no settled-fork drift. Only settled/next-fork behavioral commit is #5614 (max-blobs gossip validated from block.slot not state.slot, EIP-7892 BPO) — Lodestar validateGossipBlock already uses computeEpochAtSlot(block.slot), no drift. #5619 NewPayloadRequest dataclass→SSZ Container is value-preserving (JSON-identical per author). Rest Gloas/Fulu active-area (#5629/#5612/#5608/#5613). No PR.*
*Last updated: 2026-09-18 — master churn survey since 2026-09-11 (only 2 spec-touching commits @ origin/master 818bbed729): no settled-fork drift. #5620 (saturating_sub helper) swaps max()/min()/underflow-guard idioms across phase0/altair/fast-confirmation — hand-verified value-preserving on all 8 settled-fork substitutions incl. compute_shuffling_dependent_slot (≡ #5515 genesis-root branch). #5636 capella comment fix (cosmetic). Rest (#5640/#5631/#5627/#5633/#5632) = test-gen/CI/deps infra, no specs/ surface. No PR.*
*Last updated: 2026-09-25 — master churn survey since 2026-09-18 (14 spec-touching commits @ origin/master caa9c8e80): no settled-fork drift. Big broad touch = ms-time migration (#5667 Store.time→time_ms, #5668 genesis_time→genesis_time_ms, #5671 time-helper signatures + compute_slot_at_time_ms family, #5675) — value-preserving units refactor for sub-second BPS timing; Lodestar drives FC by slot ticks w/ native ms clock, no Store.time field to port. #5509 get_head filtered-tree Dict→node-Sequence refactor value-preserving for settled forks (payload-status variants = Gloas-only); protoArray uses viability flags. #5660 WS-period return-type Uint64→Epoch, #5659 inline local, #5678/#5682 doc/config renames — cosmetic. Behavioral churn all Gloas/Heze/future (#5665/#5663/#5652/#5605, #5592 EIP-8198 quick slots). No PR.*
