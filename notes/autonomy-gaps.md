...
1. **Add `claude-cli/*` fallback to 4 Codex-only crons + fix `cron-health-watchdog` delivery** — `self-improvement-audit-daily`, `cron-health-watchdog`, `nightly-memory-consolidation`, and `check-openclaw-release` all run a Codex-subscription-backed model (`openai/gpt-5.5` or `codex/gpt-5.3-codex-spark`) with no cross-provider fallback, and went silently unhealthy during a shared quota-exhaustion window, 2026-08-06 17:01 UTC → 2026-08-08 ~04:00 UTC (full detail: 2026-08-08 Audit workflow entry above). Awaiting Nico's sign-off on the config change (DMed 2026-08-08 ~09:4x UTC) — this is a cron-config edit, not something to patch unilaterally. Once applied: confirm via `cron.list` that `consecutiveErrors` resets to 0 on each of the four jobs, and re-run `python3 scripts/notes/check-autonomy-audit-cadence.py --require-current --fail-on-gap` on the next daily cycle to confirm the snapshot-cadence gap doesn't recur.
     ...
     **Carried-forward status (as of 2026-09-25):** ask unchanged, now 48 days pending Nico's sign-off (asked 2026-08-08). Day-by-day history 2026-08-09 → 2026-09-19 (45 entries total, archived across five passes: 26 entries 08-09→09-02 on 09-06, 6 entries 09-03→09-07 on 09-13, 5 entries 09-08→09-13 on 09-18, 2 entries 09-14→09-15 on 09-21, 4 entries 09-16→09-19 on 09-25) lives verbatim in `notes/autonomy-gaps-archive.md`. Live section below now keeps only the last ~5 calendar days (09-20 onward); consult the archive file for anything older.
   ...
   - **2026-09-25 09:3x UTC re-check (`next-audit-priorities-reminder`) — yesterday's checkable prediction confirmed exactly and the fix is committed; the live watchdog and the audit toolchain are now in git; new drift found: the workspace repo has no remote and the newest backup tarball is from 06-15:** live `cron list --json` (22 jobs) confirms the config gap is unchanged for the 4 tracked jobs (no `fallbacks`; `delivery.mode: none` on all four incl. `cron-health-watchdog`; `nightly-memory-consolidation` still `timeoutSeconds: 900`) — **48 days pending** Nico's sign-off (asked 2026-08-08). Only `nightly-memory-consolidation` is red (`consecutiveErrors=18`, `error`); the other 21 jobs are at 0. Only `Devnet health monitor`, `daily-summary` and `daily-journal` carry `payload.fallbacks`.
     **Prediction check (09-24 bullet): confirmed.** `memory/memory-cycle-2026-09-25.log`: pass 1 hit the 30-min cap (`Embedded 1056 chunks from 13 documents in 30m 46s`, `616 chunks failed`), `Step 4 (embed pass 2)` fired and embedded `77 chunks from 1 documents in 2m 0s`, pass 3 correctly did not run, and Step 4b reported exactly 2 incomplete hashes — `memory-bank/facts.md` (165 chunks, 729 KB tail) and `workspace-core/state.md` (87 chunks, 54 KB tail), the known mid-file partials a re-run cannot repair. So the second pass recovers docs the cap hit *before* starting, not the mid-file tails; the durable fix is still workload reduction (`facts.md` is regenerated nightly, so its hash changes and it is re-embedded in full — candidates unchanged from 09-24). The cron still reads `error` (900s harness vs a 33-min pipeline that finished 04:04:11Z).
     **Committed (local, GPG-signed, `lodekeeper`):** `346627c` the verified embed-pass block in `scripts/memory/nightly_memory_cycle.sh` (+18); `c64749d` `scripts/cron/check_cron_health.py` (+607/−5; the live watchdog, last commit 2026-03-06) after a review pass — `py_compile`, secret-pattern scan clean, all 4 added `subprocess.run` calls read-only (`openclaw cron list|runs`, the QMD-completeness and audit-cadence checkers), smoke run against a scratch `CRON_HEALTH_STATE_PATH` — plus a fix for a regression `346627c` itself introduced: the "embedding step took" detail read only the last pass (`2m0s` today) and now sums passes (`32m46s across 2 passes`; single-pass logs 09-22/09-24 parse unchanged); `88304a8` the 17 audit-toolchain scripts `scripts/notes/*.py|*.sh` — **never in git before** (0 tracked files there, and the watchdog now shells out to `check-autonomy-audit-cadence.py`), code only: no `__pycache__`, `review-findings/`, `review-reports/` or `state/` data; `2da4e96` `scripts/memory/check_qmd_embedding_completeness.py` (369 lines, opens the index `mode=ro`) — a dependency of both `346627c` and `c64749d` that was also untracked, caught by a post-commit `git status` check and committed as a follow-up.
     **New drift, needs Nico's call (not fixed):** the workspace repo `main` has **no remote**, its last bulk sync was 2026-03-21, and the newest same-disk `*-openclaw-backup.tar.gz` in `/home/openclaw` is 2026-06-15 (5.7 GB; disk 87% used). Right now 22 files are modified and 380 untracked entries exist, incl. `notes/autonomy-gaps.md` (~2,500 changed lines vs its last commit, 2026-05-12) and `BACKLOG.md`; I found no second copy of work since mid-June. Not doing a bulk commit from an unattended cron (much of it is `.tmp-*` scratch that stays untouched) and not adding a remote (that would leave the machine). Proposal: a private remote, or a scheduled tolerant-tar to off-disk storage.
     **Gateway:** since 09-24 09:31 UTC **0** `IsGraphAsync` FATALs / **0** `status=6/ABRT` and no gateway restart; with the 09-22/09-23/09-24 checks that is ~102h clean since the 09-21 03:09:51Z crash. That rules out the 09-18→09-21 daily-crash rate (e^-4.3 ≈ 1%) but is unremarkable at the 47-day average of 0.28/day (P ≈ 30%). The precursor volume did not fall alongside it (1,942 failover decisions and 8,337 compact-404 lines vs 658 / 2,594 in the prior window), so the burst→crash link is looking weaker, not proven wrong. Heartbeat lever still unapplied.
     **Housekeeping:** archived the 09-16 → 09-19 bullets (14 lines, verbatim) to `notes/autonomy-gaps-archive.md`; live section keeps 09-20 onward. Files touched: this file, `notes/autonomy-gaps-archive.md`, `BACKLOG.md`, plus the 4 commits above. No `memory/*.md` edited (hard constraint honored). Scratch left in `/tmp` (mine, untouched): `.cron-list-audit-0925.json`, `.cron-health-smoke-0925.out`, `.cron-health-state-audit-0925*.json`. Posted routine status to topic #347.
