...
1. **Add `claude-cli/*` fallback to 4 Codex-only crons + fix `cron-health-watchdog` delivery** — `self-improvement-audit-daily`, `cron-health-watchdog`, `nightly-memory-consolidation`, and `check-openclaw-release` all run a Codex-subscription-backed model (`openai/gpt-5.5` or `codex/gpt-5.3-codex-spark`) with no cross-provider fallback, and went silently unhealthy during a shared quota-exhaustion window, 2026-08-06 17:01 UTC → 2026-08-08 ~04:00 UTC (full detail: 2026-08-08 Audit workflow entry above). Awaiting Nico's sign-off on the config change (DMed 2026-08-08 ~09:4x UTC) — this is a cron-config edit, not something to patch unilaterally. Once applied: confirm via `cron.list` that `consecutiveErrors` resets to 0 on each of the four jobs, and re-run `python3 scripts/notes/check-autonomy-audit-cadence.py --require-current --fail-on-gap` on the next daily cycle to confirm the snapshot-cadence gap doesn't recur.
     ...
     **Carried-forward status (as of 2026-09-26):** ask unchanged, now 49 days pending Nico's sign-off (asked 2026-08-08). Day-by-day history 2026-08-09 → 2026-09-19 (47 entries total, archived across six passes: 26 entries 08-09→09-02 on 09-06, 6 entries 09-03→09-07 on 09-13, 5 entries 09-08→09-13 on 09-18, 2 entries 09-14→09-15 on 09-21, 4 entries 09-16→09-19 on 09-25, 2 entries 09-20→09-21 on 09-26) lives verbatim in `notes/autonomy-gaps-archive.md`. Live section below now keeps only the last ~5 calendar days (09-22 onward); consult the archive file for anything older.
   ...
   - **2026-09-26 09:3x UTC re-check (`next-audit-priorities-reminder`) — 09-25's prediction confirmed again; the nightly embed workload is now attributed byte-by-byte and `bank/facts.md` (the last incomplete hash) is kept out of QMD:** live `cron list --json` (22 jobs) confirms the config gap is unchanged for the 4 tracked jobs (no `fallbacks`; `delivery.mode: none` on all four incl. `cron-health-watchdog`; `nightly-memory-consolidation` still `timeoutSeconds: 900`) — **49 days pending** Nico's sign-off (asked 2026-08-08). Only `nightly-memory-consolidation` is red (`consecutiveErrors=19`, `lastDurationMs=901299`); the other 21 jobs are at 0; only `Devnet health monitor`, `daily-summary` and `daily-journal` carry `payload.fallbacks`.
     **Prediction check (09-25 bullet): confirmed.** `memory-cycle-2026-09-26.log` (03:30:48Z → 04:04:41Z, 33m53s): pass 1 `Embedded 1248 chunks from 12 documents in 30m 28s`, `540 chunks failed` (the cap); `Step 4 (embed pass 2)` fired: `137 chunks from 4 documents in 3m 5s`; Step 4b: **1** incomplete hash (was 2) — `memory-bank/facts.md`, 297 chunks, 529 KB tail (`state.md` recovered). Embedding is 33m13s of the 33m53s; Steps 1-3, `qmd update`, the status probes and 4b/5 together are ~20s, and the per-invocation CUDA-probe noise in the log costs ~2.8s (`time qmd status`), so there is nothing to win there.
     **Workload attribution (QMD index, read-only):** the 09-26 night embedded 3.6 MB / 1,788 chunks at ~2.0 KB/s CPU: `BACKLOG_ARCHIVE.md` 1.33 MB (712 chunks; the BACKLOG cleanup rewrites it daily, so the whole file re-embeds every night — today's 08:52Z edit already re-queues it), `bank/facts.md` 0.97 MB (~700 chunks; regenerated at 03:30 nightly) and ~1.3 MB across the other 10 docs. The static giants (`backlog-recovery-staging.md` 618 KB, `state-archive.md` 332 KB, the April dailies) cost nothing nightly. To fit the 900s harness limit the nightly load would have to drop under ~1.6 MB, i.e. both giants out; `facts.md` alone (27%) does not get there.
     **Change applied (QMD config, not OpenClaw config):** `~/.config/qmd/index.yml` `memory-bank` pattern `**/*.md` → `**/!(facts).md` (one line; backup `/tmp/.qmd-index.yml.bak-0926`; rollback = restore that line, the next `qmd update` re-adds `facts.md` and it re-embeds in one night). Verified before/after: fast-glob against the real dir keeps 333 of 334 bank files (only `facts.md` dropped); live `qmd update` → `bank: 333 unchanged, 1 removed`, docs 961 → 960, vectors unchanged, `Pending: 3` (today's edited docs, expected). Why low-loss: `rebuild_index.py` already skips the generated facts/decisions/preferences/lessons views in favour of `state.json`, and `query_index.py --kind fact` still returns facts (checked); the daily notes the facts are extracted from, and the other bank views, stay in QMD. Cost: `facts.md` chunks no longer appear in `qmd search/query`. Comment-only script note committed as `f6b53cc` (the exclusion itself lives in the untracked qmd config).
     **Checkable prediction for 09-27:** with `facts.md` gone the same doc set is ~2.6 MB (2.2-2.6 MB over the last five nights minus 0.97), i.e. ~22-26 min at 2.0-1.7 KB/s — inside the 30-min cap with a thin margin — so expect `memory-cycle-2026-09-27.log` to show pass 1 with **0 failed chunks**, **no** `embed pass 2`, and Step 4b `QMD_EMBEDDING_COMPLETE` (0 incomplete hashes). The cron still reads `error` (~22-26 min vs 900s) until the 2700s/detach change lands. If the cap still bites, the miss is `BACKLOG_ARCHIVE.md`-sized. **Not applied:** taking `BACKLOG_ARCHIVE.md` out too (would put the pipeline at ~11 min, under 900s) — it is the only copy of the archived investigation logs the BACKLOG entries point to and `query_index.py` does not index it, so exclusion costs real recall; the recall-preserving alternative is rotating it by month into an indexed, static path (embedded once), which needs the cleanup cron's append target changed — Nico's call, or the 2700s timeout.
     **Gateway:** since 09-25 09:31 UTC **0** `IsGraphAsync` FATALs / **0** `status=6/ABRT`; one clean stop/start at 09-25 19:29:15Z (not a crash, not investigated). ~126h clean since the 09-21 03:09:51Z crash (e^-1.47 ≈ 23% at the 0.28/day average, so still unremarkable); 1,963 failover decisions / 8,398 compact-404 lines in the window vs 1,942 / 8,337 the day before, so the burst→crash link stays unproven. Heartbeat lever still unapplied.
     **Housekeeping:** archived the 09-20 and 09-21 bullets (10 lines, verbatim) to `notes/autonomy-gaps-archive.md`; live section keeps 09-22 onward. Files touched: this file, `notes/autonomy-gaps-archive.md`, `BACKLOG.md`, `scripts/memory/nightly_memory_cycle.sh` (comment, `f6b53cc`), `~/.config/qmd/index.yml`. No `memory/*.md` edited (hard constraint honored). Scratch left in `/tmp` (mine, untouched): `.cron-list-audit-0926.json`, `.gw-journal-audit-0926.txt`, `.qmd-update-0926.log`, `.qmd-index.yml.bak-0926`, `.globtest-0926.mjs`, `.autonomy-gaps*.bak-0926`. Posted routine status to topic #347.
