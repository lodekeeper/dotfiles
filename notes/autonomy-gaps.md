...
> Updated: 2026-09-25 (155th pass)

---

## Daily Audit Snapshot — 2026-09-25 (self-improvement-audit-daily, 03:19 UTC)
...
### Audit workflow
- **Status:** structured domain-preflight health baseline initialized (21 check(s)) in `/home/openclaw/.openclaw/workspace/state/autonomy-domain-preflight-health.json`. Proposed fix: future audits now compare structured preflight health before treating unchanged prose statuses as `NO_REPLY`.

---
## Daily Audit Snapshot — 2026-09-24 (self-improvement-audit-daily, 03:17 UTC)
...
---
## Daily Audit Snapshot — 2026-09-21 (self-improvement-audit-daily, 03:17 UTC)
...
1. **Add `claude-cli/*` fallback to 4 Codex-only crons + fix `cron-health-watchdog` delivery** — `self-improvement-audit-daily`, `cron-health-watchdog`, `nightly-memory-consolidation`, and `check-openclaw-release` all run a Codex-subscription-backed model (`openai/gpt-5.5` or `codex/gpt-5.3-codex-spark`) with no cross-provider fallback, and went silently unhealthy during a shared quota-exhaustion window, 2026-08-06 17:01 UTC → 2026-08-08 ~04:00 UTC (full detail: 2026-08-08 Audit workflow entry above). Awaiting Nico's sign-off on the config change (DMed 2026-08-08 ~09:4x UTC) — this is a cron-config edit, not something to patch unilaterally. Once applied: confirm via `cron.list` that `consecutiveErrors` resets to 0 on each of the four jobs, and re-run `python3 scripts/notes/check-autonomy-audit-cadence.py --require-current --fail-on-gap` on the next daily cycle to confirm the snapshot-cadence gap doesn't recur.
     ...
     **Carried-forward status (as of 2026-09-21):** ask unchanged, now 44 days pending Nico's sign-off (asked 2026-08-08). Day-by-day history 2026-08-09 → 2026-09-15 (41 entries total, archived across four passes: 26 entries 08-09→09-02 on 09-06, 6 entries 09-03→09-07 on 09-13, 5 entries 09-08→09-13 on 09-18, 2 entries 09-14→09-15 on 09-21) lives verbatim in `notes/autonomy-gaps-archive.md`. Live section below now keeps only the last ~5 calendar days (09-16 onward); consult the archive file for anything older.
   ...
   - **2026-09-18 09:3x UTC re-check (`next-audit-priorities-reminder`) — largest live recurrence yet of the exact incident that motivated this ask (8/22 jobs down simultaneously), plus the first genuinely skipped (not harness-timeout-masked) `nightly-memory-consolidation` run in this tracking arc:** live `cron list --json` confirms the config gap is still unchanged for the 4 originally-tracked jobs (no `fallbacks` field in any payload, confirmed via full raw JSON dump, not just the summary view; `cron-health-watchdog` still `delivery.mode: none`; `nightly-memory-consolidation` still `payload.timeoutSeconds: 900`) — **41 days pending** Nico's sign-off (asked 2026-08-08). Nico DM thread (`sessions_list` on `agent:main:telegram:direct:5774760693`): `updatedAt` last advanced to 2026-09-15T16:03:27Z with `lastMessagePreview: "NO_REPLY"` (a routine automated reply, not Nico-initiated content) — no new decision-relevant fact since, no re-DM (calibrated-restraint precedent, unchanged).
     **Fleet-wide scan (22 jobs) found 8 with `consecutiveErrors > 0` — the widest blast radius seen in this tracking arc; prior peak was 2 simultaneous real jobs (09-16: `nightly-memory-consolidation` + `daily-journal`):** `monitor-open-pr-ci` (23), `cron-health-watchdog` (22), `nightly-memory-consolidation` (11), `workspace-periodic-cleanup` (7), `Sync dotfiles repo` (6), `ci-autofix-unstable` (5), `check-openclaw-release` (4), `self-improvement-audit-daily` (4). Every one carries `lastErrorReason: rate_limit` and the identical root message: "You've reached your Codex subscription usage limit... reset [at] Sep 19 at 8:18 AM UTC" — a live, ongoing, shared-quota-exhaustion window, earliest observed failure at 2026-09-17T22:20:30Z (`check-openclaw-release`), most recent at 2026-09-18T09:12:03Z (`monitor-open-pr-ci`, ~20min before this check) — **still actively failing at the moment of this check**, 11+ hours in. This is the exact incident class that produced the original 2026-08-08 ask (2026-08-06→08-08 shared quota exhaustion), now recurring at 4x the prior peak job count.
     **New wrinkle that sharpens the ask itself:** the 5 jobs configured with `codex/gpt-5.3-codex-spark` as primary (`cron-health-watchdog`, `check-openclaw-release`, `monitor-open-pr-ci`, `Sync dotfiles repo`, `workspace-periodic-cleanup`) show `lastDiagnosticSummary: "All models failed (2)"` — a second model, `openai/gpt-5.5`, was attempted automatically even though none of these jobs' payloads declare a `fallbacks` field. So some fallback-attempt mechanism already exists at the platform level independent of per-job config — but it provides no real cross-provider resilience: `openai/gpt-5.5` failed with the identical "Codex subscription usage limit" message, meaning it draws from the *same* Codex/ChatGPT account quota as the primary. A same-account "fallback" cannot help during a shared-quota outage — first-hand, concrete evidence that the ask's specific requirement (`claude-cli/*`, a genuinely different provider/account) is the right fix, not just "any fallback." Also reconfirms the 09-16 finding: the `gpt-5.3-codex-spark` 400 "not supported when using Codex with a ChatGPT account" error (a distinct, non-rate-limit failure mode) recurs today across all 5 spark-primary jobs, layered on top of the quota exhaustion.
     **`nightly-memory-consolidation`: first genuinely skipped run in this multi-week arc, not the usual harness-timeout-masks-a-completed-pipeline pattern.** `lastDurationMs=4958` (~5s) and no `memory/memory-cycle-2026-09-18.log` was created at all — the agent turn failed on the rate-limit error before the bash script could even be invoked. Every prior "failure" tracked here since 09-04 was a real completion mislabeled `error` by the 900s harness timeout; today there is no completion to find. Impact is low and expected to self-heal (the consolidation script processes accumulating daily notes, not date-locked snapshots, so the next successful run picks up 09-17+09-18's notes together) — but flagging a specific, checkable prediction for tomorrow's cycle: both `nightly-memory-consolidation` and `self-improvement-audit-daily` are now back on their normal once-daily schedule (backoff ladder already exhausted from these consecutive-error streaks, same mechanism root-caused 09-13), with next attempts at 2026-09-19 03:30:00Z and 03:16:29Z respectively — **both roughly 5 hours *before* the stated 08:18 UTC quota reset.** Unless the reset lands earlier than advertised, expect both to fail again tomorrow morning before self-healing on a later same-day retry or the 09-20 run.
     **`cron-health-watchdog` confirmed still structurally blind to its own failure:** manually ran `scripts/cron/check_cron_health.py` — it correctly listed 7 of the 8 failing jobs (all delivery `not-requested`, so none of this would surface without a manual check, reconfirming the delivery half of the ask), but omitted `cron-health-watchdog` itself (`consecutiveErrors=22`) from its own output — the same self-blind-spot first documented 2026-09-10. No config changes applied (still gated on Nico's sign-off); the 09-13 scoped commands remain the exact actionable path. **Housekeeping this cycle:** archived the 2026-09-08→09-13 day-by-day entries (5 sub-bullets) to `notes/autonomy-gaps-archive.md` — this live section had grown back to 9 dated bullets, the same "re-approaching risky scale" trigger point noted 09-13; keeping only 09-14 onward inline per the standing ~5-day rule. Files touched: this file, `notes/autonomy-gaps-archive.md`, `BACKLOG.md`. No `memory/*.md` files edited (hard constraint honored). Posted routine status to topic #347.
   - **2026-09-19 09:4x UTC re-check (`next-audit-priorities-reminder`) — yesterday's checkable prediction confirmed exactly, the quota reset is confirmed landed (failing jobs 8 → 5), and the same exhaustion window gave the first controlled comparison for the fallback ask:** live `cron list --json` confirms the config gap is still unchanged for the 4 originally-tracked jobs (`fallbacks` absent on all four; `delivery.mode: none` on all four incl. `cron-health-watchdog`; `nightly-memory-consolidation` still `timeoutSeconds: 900`; no `failureAlert` anywhere) — **42 days pending** Nico's sign-off (asked 2026-08-08). Nico DM thread (`sessions_list`): `updatedAt` still 2026-09-15T16:03:27Z, `lastMessagePreview: "NO_REPLY"` — no new Nico activity, no re-DM (this cycle adds evidence to an ask he already has; it creates no new decision).
     **Prediction check (09-18 bullet): confirmed.** `self-improvement-audit-daily` failed at 03:17:55Z (`consecutiveErrors=5`) and `nightly-memory-consolidation` at 03:30:48Z (`consecutiveErrors=12`), both `rate_limit` in 5–7s ("Next reset in 6/5 hours, Sep 19 at 8:18 AM UTC") — the agent turn died before any script ran, so `memory/memory-cycle-2026-09-18.log` and `-09-19.log` do not exist and the latest audit snapshot is still 2026-09-17. `check-autonomy-audit-cadence.py --require-current --fail-on-gap` exits 2 with a fresh latest-snapshot freshness gap (2026-09-18 missing; the 09-19 run wrote none either). **Consequence worth stating plainly:** both jobs are on their once-daily cadence (backoff ladder exhausted), so the next attempts are 2026-09-20 03:16:29Z / 03:30:00Z — ~19h *after* the reset. Two consecutive nightly consolidations and two audit snapshots are lost to a window that had already closed when the next run was due. Expected to self-heal 09-20 (the consolidation script processes accumulated notes, nothing is date-locked) provided quota holds. Deliberately **no manual `openclaw cron run`** from this unattended reminder: no precedent in `scripts/cron/` or the archive, `nightly-memory-consolidation` is a ~30-min CPU pipeline writing memory artifacts under this cron's own no-memory-edits constraint, and waiting one more cycle is cheap — but it is a one-command option if Nico wants the memory index recovered sooner.
     **Post-reset recovery: failing set 8 → 5.** Healed with no intervention after 08:18Z: `cron-health-watchdog` (09:00/09:30Z, on `openai/gpt-5.5`), `workspace-periodic-cleanup` (08:33Z), `ci-autofix-unstable` (08:44Z) — quota is back on both the `codex/*` and `openai/gpt-5.5` paths. Still red: `check-openclaw-release` (6; next tick 09:58Z, post-reset — expect it to heal), `Sync dotfiles repo` (10; next 12:01Z), the two dailies above, and `monitor-open-pr-ci` (1) — the last is a **different cause, not quota**: "cron: job interrupted by gateway restart" at ~09:29:40Z. Watchdog note: its 09:30:59Z run *did* detect that failure, but delivery is `not-requested` — the delivery half of the ask is still unmet, so even a correctly-detected failure reaches nobody.
     **New evidence that sharpens the fallback ask (first controlled comparison):** during the same exhaustion window (earliest failure 2026-09-17T22:20:30Z → reset 09-19 08:18Z), `daily-journal` and `daily-summary` — the two jobs that *already* carry a `claude-cli/*` fallback — ran at 2026-09-18 23:00:39Z and finished OK at 23:02:47Z with `provider: claude-cli` (`claude-opus-4-8` / `claude-sonnet-4-6`, per `cron_run_logs`), `consecutiveErrors=0`. Same cause, same window, same account; the only difference from the Codex-only jobs that failed is a cross-provider fallback. That is direct in-fleet proof the requested config works — stronger than the 09-18 inference that a same-account `openai/gpt-5.5` fallback cannot help. Calibration caveat (from the 09-16 bullet): a fallback only helps while the fallback provider has its own headroom — `daily-journal` failed repeatedly that week when `claude-cli/claude-opus-4-8` was itself rate-limited until 16:00 UTC. Files touched: this file, `BACKLOG.md`. No `memory/*.md` files edited (hard constraint honored). Posted routine status to topic #347.
   - **2026-09-20 09:4x UTC re-check (`next-audit-priorities-reminder`) — "harness timeout is cosmetic" is retracted for last night: first genuinely incomplete `nightly-memory-consolidation` run, root-caused to a recurring gateway crash (SIGABRT ×3 in 3 days):** live `cron list --json` confirms the config gap is unchanged for the 4 tracked jobs (no `fallbacks`, `delivery.mode: none`, nightly `timeoutSeconds: 900`) — **43 days pending** Nico's sign-off. Yesterday's prediction, partly confirmed: the quota window is over and 7 of the 8 jobs red on 09-18 healed with no intervention (red set 8 → 5 → 2; `self-improvement-audit-daily` ran 03:16Z ok and wrote the 09-20 snapshot), but `nightly-memory-consolidation` (03:30Z, `consecutiveErrors=13`) did not: `memory-cycle-2026-09-20.log` stops mid-`qmd embed` at 44% (832/1768 chunks), 03:48:18, no completion marker. The watchdog's marker detector reported exactly that ("no completion marker… timeout may reflect a real incomplete run") — first live hit of its negative branch, so the detector is validated, not buggy.
     **Root cause (user journal + `/tmp/openclaw/openclaw-2026-09-20.log`):** the harness aborted the agent at 03:45:00 (900s) but the pipeline kept running as an orphan under the gateway service; `openclaw-gateway.service` then died at 03:48:42 — `code=dumped, status=6/ABRT`, `FATAL ERROR: v8::Module::IsGraphAsync v8::Module::IsGraphAsync must be used on an instantiated module` from `node::loader::ModuleWrap::InstantiateSync` (Node's `require(esm)` path; gateway runs `/usr/bin/node` v22.19.0; the CLI's nvm Node is v22.22.0) — and `KillMode=control-group` took the whole cgroup, pipeline included. Restarted in 5s (`Restart=always`, counter 3, `recovered=4` sessions). **The identical FATAL line is in all three crashes: 09-18 02:59:53, 09-19 09:29:50 (that is the "job interrupted by gateway restart" that hit `monitor-open-pr-ci`), 09-20 03:48:42**; none in the journal 09-10 → 09-17 [**CORRECTED 2026-09-21 — wrong: 13 identical crashes since 08-06, including 09-11 22:28:04; see the 09-21 bullet**] (earlier restarts were deliberate stops / SIGKILL-on-stop-timeout). So the old framing ("pipeline completed, timeout is cosmetic") was luck-dependent: a pipeline that outlives its cron session is exposed to any gateway death during the 15–30 min after the timeout, and with a crash now roughly daily that exposure is real. This sharpens the timeout half of the ask (2700s keeps the run inside its session, so a crash shows up as a real error, not "timeout") but does not fix the crash.
     **Impact now:** `scripts/memory/check_qmd_embedding_completeness.py` (opens the index `mode=ro`) reports 20 incomplete hashes — daily notes 09-17/09-18, `memory-bank/{decisions,preferences}.md` all-missing, `facts.md` tail, several entity pages, `workspace-core/{backlog,backlog-archive,identity}.md`. Keyword search is fine (Step 3 / `qmd update` ran); vector/hybrid is stale until the next run. Deliberately no manual re-embed: a 30+ min CPU job midday starves the gateway (ELU 0.97 / loop delay P99 1.9s during last night's embed) and the 900s-limited crons. Checkable prediction for the next cycle: 09-21 03:30Z resumes the embed and, at ≥1768 chunks, again outlives 900s — and is exposed to another crash.
     **Second red job, different cause:** `beacon-log-monitor` (4 consecutive timeouts 09-19 12:30–12:55Z, `last phase: process-spawned`, 24h cadence, `claude-cli/sonnet`). `monitor-beacon.sh` ran clean in 1s with normal low-severity REST-404 noise, and 27 gateway liveness/stuck-session log lines fall in 12:20–12:59 that day — consistent with gateway starvation, but unproven (`github-notifications` ran fine in the same window). Next run 12:18Z today; expect heal, recheck next cycle. Not part of the fallback ask. Candidate mitigation for Nico, **not applied** (service change, his call): run the gateway unit on a newer Node — unverified that it fixes this. [**CORRECTED 2026-09-21: evidence now points against this (crash site identical through Node 22.22.0); see the 09-21 bullet for the better-supported lever.**] Housekeeping deferred (09-14/09-15 bullets to archive next cycle, cron budget). Files touched: this file, `BACKLOG.md`. No `memory/*.md` files edited (hard constraint honored). Posted routine status to topic #347.
   - **2026-09-21 09:3x UTC re-check (`next-audit-priorities-reminder`) — the gateway SIGABRT is chronic (13× since 08-06, not "3× in 3 days"), every one lands inside the 10-min sub-agent heartbeat burst, and "newer Node" is retracted as a mitigation:** live `cron list --json` (22 jobs) confirms the config gap is unchanged for the 4 tracked jobs (no `fallbacks`; `delivery.mode: none` on all four incl. `cron-health-watchdog`; `nightly-memory-consolidation` still `timeoutSeconds: 900`) — **44 days pending** Nico's sign-off (asked 2026-08-08). Only three other jobs carry a `claude-cli/*` fallback (`daily-journal`, `daily-summary`, `Devnet health monitor`). Fleet scan: exactly one job red, `nightly-memory-consolidation` (14 errors; its 03:30Z run again hit the 900s harness limit, `lastDurationMs=901206`); `self-improvement-audit-daily` healed (0 errors, wrote the 09-21 snapshot at 03:17Z) and `beacon-log-monitor` healed (0, `ok`). The gateway was also *deliberately* restarted at 08:53:13Z today (clean SIGTERM, "shutdown completed cleanly in 241ms", `openclaw.json.last-good` rewritten 08:53) — not a crash, Nico-side, not investigated.
     **09-20's prediction, checked (right on the outcome shape, wrong on timing):** the next crash did come (09-21 03:09:51Z, 4th in 4 days) but ~20 min *before* the 03:30Z pipeline started, so the nightly pipeline was not killed: `memory-cycle-2026-09-21.log` runs to "Nightly memory cycle complete" at 04:00:52Z, but `qmd embed` reports `Embedded 1056 chunks … in 30m 25s` with `575 chunks failed` (`SessionReleasedError: LLM session has been released or aborted` — same error shape as the 09-14 storm; cause not investigated). QMD incompleteness is down 20 → 7 hashes (`daily-notes/2026-09-17` + `2026-09-21`, `eth-rnd-archive-notes/2026-09-18`, `memory-bank/entities/prs/pr-10096`, `memory-bank/facts.md` tail, `memory-bank/preferences.md`, `workspace-core/backlog.md`); keyword search is fine; the next run retries the remainder, so still no manual re-embed (same CPU-starvation reasoning as 09-20).
     **Corrections to the 09-20 bullet (two claims were wrong):** (1) The journal (starts 07-19) holds **13** identical `IsGraphAsync` FATALs, matched **1:1** by 13 `code=dumped, status=6/ABRT` gateway exits: 08-06, 08-15 ×2, 08-20, 08-28, 09-01, 09-03, 09-04, **09-11 22:28:04**, 09-18, 09-19, 09-20, 09-21. "None 09-10 → 09-17" missed 09-11, and "3× in 3 days" was the visible tail of a chronic ~1-per-4-days problem (9 crashes 08-06→09-11) that turned daily on 09-18; none before 08-06. (2) **"Newer Node for the gateway unit" is not supported by the evidence.** The crash site — `ModuleWrap::InstantiateSync` ending in an unguarded `module->IsGraphAsync()` — is line-for-line identical in nodejs/node v22.19.0 / v22.20.0 / v22.21.0 / v22.22.0 (raw `src/module_wrap.cc` at each tag via `gh api`; the only change is `obj->resolve_cache_.clear()` dropped in ≥22.21), and `lib/internal/modules/esm/module_job.js` v22.19.0→v22.22.0 differs only by `link()`-API/constructor refactors with no added status guard. The sibling JS assertion (`ERR_INTERNAL_ASSERTION … module that is imported again after being required, Status = 0` — same "require(esm) meets an uninstantiated module" state) is open on openclaw/openclaw#114176 (4/4 repro on Node v22.23.1) and #116164 (Node v24.18.0/24.18.1). The one root-cause lead in those threads — stale duplicate `__openclaw-generation__` plugin dirs — does not apply here (`~/.openclaw/npm/projects/` has exactly one dir per plugin, 5 total), and there are zero non-fatal `ERR_INTERNAL_ASSERTION` lines in the journal since 09-10. Swapping the unit's `ExecStart=/usr/bin/node` (v22.19.0, Aug-2025 build) for nvm's v22.22.0 is therefore a coin-flip at best; do not present it as the fix.
     **What the crashes do share (quantified, not eyeballed):** a random 12s window over 08-06→09-21 contains an `embedded run failover decision` line only **8.9%** of the time (8.1% for the Codex `Error running remote compact task … 404` line), yet **13/13** crash windows ([T−12s, T]) contain a failover line and 12/13 the compact-404 line. Source: `agents.defaults.heartbeat = {every: "10m", target: "last", directPolicy: "allow"}` with **no per-agent `heartbeat` block**, so all 12 agents heartbeat. 7 sub-agents are configured `openai/gpt-5.5` → `openai/gpt-5.4-pro` on the Codex runtime (`codex-reviewer`, `gpt-advisor`, `review-bugs`, `review-defender`, `review-security`, `reviewer-architect`, `devils-advocate`); Codex remote compaction (`chatgpt.com/backend-api/codex/responses/compact`) is logged returning 404 on nearly every day since 08-06 (first FATAL and first compact-404 line are the same day; neither appears 07-25→08-05). Measured 09-19: six of those lanes (all but `devils-advocate`) error on a ~600s median gap — exactly the heartbeat — i.e. ~1,500–2,000 failover lines/day of doomed runs for six weeks. **Association is strong; mechanism is unproven** (plausible, untested: several failing runs fail over in the same second and lazily `require()` the same ESM provider module).
     **Lever for Nico (config edit — NOT applied):** `docs/gateway/heartbeat.md:120,127` — "if any `agents.list[]` entry has a `heartbeat` block, only those agents run heartbeats" (per-agent block merges over defaults) — so adding a `heartbeat` block to the `main` agent only (e.g. `{"every": "10m"}`) stops the 11 sub-agents heartbeating. Test: `journalctl --user -u openclaw-gateway | grep -c IsGraphAsync` stays 0 and the daily failover-line count drops to ~0; judge it over ≥10 crash-free days at the 47-day average rate (0.28/day, p≈0.06), or ~3 days if the recent daily rate persists. `gemini-reviewer` also errors every ~3 min (283 lane errors on 09-19) — same fix covers it.
     **Housekeeping:** archived the 2026-09-14 and 2026-09-15 day-by-day bullets (verbatim, 8 lines) to `notes/autonomy-gaps-archive.md` — the item deferred on 09-20; live section keeps 09-16 onward. Files touched: this file, `notes/autonomy-gaps-archive.md`, `BACKLOG.md`. Scratch left in `/tmp` (mine, untouched): `.gw-journal-audit-0921.txt`, `.cron-list-audit-0921.json`. No `memory/*.md` files edited (hard constraint honored). Posted routine status to topic #347 (messageId 13902) — by the 09:47Z retry: the 09:30Z run timed out at 900s right after writing this bullet, before delivery, and its background verifier never reported. The retry independently re-derived the 13 FATAL ↔ 13 ABRT count/dates, the 13/13 (failover) and 12/13 (compact-error) crash-window association against a ~8% base rate (from the user journal, since `/tmp/openclaw` daily logs only survive for 09-20/21), and the no-per-agent-`heartbeat` config + docs lines 120/127; the Node-source-diff, 09-19 lane-cadence and 09-21 embed/QMD figures were **not** re-derived. Retry scratch (mine, untouched): `/tmp/.gwj-retry-0921.txt` (user-journal dump).
   ...
   - **2026-09-24 09:3x UTC re-check (`next-audit-priorities-reminder`) — fleet healed 7 → 1 red, second crash-free window, and the nightly `qmd embed` `SessionReleasedError` storm is root-caused (a hard 30-min session cap, not a bug) and fixed script-side:** live `cron list --json` (22 jobs) confirms the config gap is unchanged for the 4 tracked jobs (no `fallbacks`; `delivery.mode: none` on all four incl. `cron-health-watchdog`; `nightly-memory-consolidation` still `timeoutSeconds: 900`; `~/.openclaw/openclaw.json` mtime still 2026-06-15, no per-agent `heartbeat` block) — **47 days pending** Nico's sign-off (asked 2026-08-08). Fleet: 21/22 at `consecutiveErrors=0`; only `nightly-memory-consolidation` red (17, `timeout`, 900641ms). All six other 09-23 red jobs self-healed with no intervention. Gateway: since 09-23 09:31 UTC **0** `IsGraphAsync` FATALs and **0** `status=6/ABRT` (last crash 09-21 03:09:51Z, ~54h clean); failover decisions 1,760 → 658 and compact-404 lines 9,256 → 2,594 with the heartbeat config unchanged — a data point for, not proof of, the heartbeat hypothesis (the only event in the window is the clean 09-23 08:53 restart).
     **QMD root cause (was "cause not investigated" 09-14 → 09-23):** `@tobilu/qmd` 1.0.7 `dist/qmd.js:1439` wraps the whole embed in `withLLMSession(…, {maxDuration: 30*60*1000})`; `dist/llm.js:905-947` aborts the session at the cap and every later `session.embed*` throws `SessionReleasedError`. Nightly workload is 3.2–3.6 MB / 1,631–1,768 chunks (cycle logs 09-20/21/22/24) against ~2.3–2.7 MB per session at CPU speed (~1.9 KB/s), so every night ends at 30m16–30m25s with 458–842 chunks failed (09-21: 575, 09-22: 842, 09-24: 458). Biggest loads: `bank/facts.md` (943 KB / 697 chunks, regenerated 03:32 nightly by the pipeline's own Step 1/2), `BACKLOG_ARCHIVE.md` (1.23 MB / 315+ chunks), `bank/preferences.md`. Selection is `LEFT JOIN content_vectors ON hash AND seq=0 … IS NULL` (`store.js:1665`): docs the cap hit *before* starting (no seq 0) are re-queued next run; docs it hit *mid-file* are never re-queued until their content changes — the permanent "large-tail-after-last-vector" partials (`backlog-archive.md` 315 chunks + 666 KB tail, `state.md` 87 chunks).
     **Fix applied (script-side, no cron config):** `scripts/memory/nightly_memory_cycle.sh` now runs up to 2 follow-up `qmd embed` passes while `qmd status` shows `Pending: N need embedding` (fresh 30-min session each; bounded; qmd's own CLI only). Verified: `bash -n`; stub-`qmd` control-flow test on the block extracted from the real script (always-pending → 3 embeds/2 status checks, never → 1/1, once → 2/2); real-`qmd status` grep matches while pending and not after; and a live one-off `nice -n 19 qmd embed` today over the 12 stuck docs (165 KB / 101 chunks) → `Embedded 101 chunks from 12 documents in 2m 20s`, 0 failed, completeness **14 → 2 incomplete hashes** (exactly the two partial giants, as predicted). **Not fixed by re-running:** those 2 partials (need a content change or `qmd embed -f`, a multi-hour job). The durable fix is workload reduction — candidates, not applied: stop regenerating `facts.md` nightly, split `BACKLOG_ARCHIVE.md` by month, or keep the giants out of embeddings. The cron still reads `error/timeout` (900s harness vs a ≥31-min pipeline) until the 2700s/detach change lands; this fix changes index completeness, not cron status. Script change is uncommitted in the working tree until tomorrow's 03:30Z run confirms it (same precedent as `17e71a4`); expect `Step 4 (embed pass 2)` in `memory-cycle-2026-09-25.log` and ≤2 incomplete hashes in its Step 4b.
     **Also found (workflow drift, not fixed here):** `scripts/cron/check_cron_health.py` carries 609 uncommitted lines (its only commit is 2026-03-06 `f1c39ed`; diff has no secret-like strings) — the live watchdog is not in git; commit after a review pass. Housekeeping still due: archive the 09-16 → 09-18 bullets above to `notes/autonomy-gaps-archive.md` (live section keeps ~5 days). Files touched: this file, `BACKLOG.md`, `scripts/memory/nightly_memory_cycle.sh`. No `memory/*.md` edited (hard constraint honored). Scratch left in `/tmp` (mine, untouched): `.cron-list-audit-0924.json`, `.gw-journal-audit-0924.txt`, `.qmd-embed-manual-0924.log`, `nightly-loop-test-x9JC/`. Posted routine status to topic #347.
